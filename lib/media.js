...
/* Plateformes reconnues au titre publié : Netflix ne publie que son propre
   nom, Prime Video publie le sien devant le titre réel. */
const PLATFORMS = [
  { needle: "netflix", name: "Netflix" },
  { needle: "prime video", name: "Prime Video" },
  { needle: "amazon", name: "Prime Video" },
  { needle: "youtube", name: "YouTube" }
];

/* À défaut de plateforme, l'application qui joue. macOS la publie sous son
   identifiant de bundle, y compris pour une lecture web — où il désigne le
   navigateur, seul nom disponible quand le site ne se nomme pas. */
const CLIENTS = {
...
/* Nom affichable de ce qui joue. On ne choisit pas la source — macOS n'expose
   qu'une session à la fois — on se contente de nommer celle qui joue, et de
   ne rien afficher quand on ne sait pas la nommer. */
function sourceFrom(data) {
  const haystack = [data.title, data.artist, data.album]
    .filter(v => typeof v === "string").join(" ").toLowerCase();

  for (const platform of PLATFORMS) {
    if (haystack.indexOf(platform.needle) !== -1) return platform.name;
  }

  const client = clean(data.clientBundleIdentifier);
  return client && CLIENTS[client] ? CLIENTS[client] : null;
}
...
async function readNow() {
  ...
  if (bin) {
    try {
      const raw = await run(bin, ["get", "--json", "title", "artist", "playbackRate", "duration", "elapsedTime", "clientBundleIdentifier"], READ_TIMEOUT);
  ...
  /* Repli AppleScript : l'application détectée est la seule chose qu'on
     sache d'elle, et elle fait un nom de source parfaitement valable. */
  const app = fallbackApp();
  current = app
    ? { available: true, playing: null, title: null, artist: null, backend: app, source: app }
    : { available: false };
  return current;
}
