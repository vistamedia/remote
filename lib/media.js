...
function run(file, args) {
  return new Promise((resolve, reject) => {
    execFile(file, args, { timeout: 5000 }, (err, stdout, stderr) => {
...
async function refresh() {
  ...
  if (bin) {
    try {
      const raw = await run(bin, ["get", "--json", "title", "artist", "playbackRate"]);
      ...
      /* Le binaire existe mais n'a rien pu dire : on garde le backend
         disponible, sans métadonnées. */
      current = { available: true, playing: null, title: null, artist: null };
    }
    return current;
  }
