 ...
 *
 * Cmd-Ctrl-F est le raccourci système du plein écran de fenêtre, et non
 * celui du lecteur vidéo — « f » seul l'aurait atteint, mais serait tombé
 * dans la page comme une frappe de texte si le curseur s'était trouvé dans
 * un champ de saisie. Le raccourci système ne peut, lui, jamais écrire quoi
 * que ce soit. */
const KEYSTROKE = `function run() {
  Application("System Events").keystroke("f", { using: ["command down", "control down"] });
  return "0";
}`;
