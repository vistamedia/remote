...
const APPS = {
  ...
  /* Firefox Developer Edition porte son propre identifiant : l'omettre
     rendait le bouton invisible sur la machine qui s'en sert. */
  "org.mozilla.firefoxdeveloperedition": { name: "Firefox", scriptable: false }
};
...
/* Les noms de variables évitent « focused » et « role », qui appartiennent
   au dictionnaire de System Events : les réutiliser fait échouer le script
   sur une erreur -10006, mesurée. */
const KEYSTROKE = `tell application "System Events"
  set axTarget to missing value
  try
    set axTarget to value of attribute "AXFocusedUIElement" of (first application process whose frontmost is true)
  on error errorMessage number errorNumber
    return "err:" & errorNumber
  end try

  if axTarget is not missing value then
    set axRole to ""
    try
      set axRole to role of axTarget
    end try
    if axRole is in {"AXTextField", "AXTextArea", "AXComboBox", "AXSearchField"} then
      return "focus"
    end if
  end if
...
/* Accessibilité constatée au dernier sondage. Conservée à part : elle
   commande la frappe de touche, mais pas la voie par la page, qui n'en a
   pas besoin. */
let trusted = false;
...
/* AppleScript classique, pour la seule garde du focus : JXA échoue à
   convertir la valeur de « AXFocusedUIElement » — erreur -1700 mesurée — là
   où AppleScript la lit sans peine. Le reste du fichier reste en JXA, seul
   capable de lire NSWorkspace. */
...
async function readNow() {
  ...
  /* Navigateur : deux voies indépendantes, la page et la frappe de touche.
     « available » est calculé au snapshot, la page pouvant répondre alors
     que l'Accessibilité manque. L'état reste inconnu dans les deux cas — le
     plein écran d'une vidéo web n'est pas celui de la fenêtre. */
...
/* Pour un navigateur, la page est une seconde voie : le bouton doit
   apparaître dès que l'une des deux répond. Le calcul se fait ici plutôt
   qu'à la lecture, la page étant interrogée après ce module. */
function snapshot() {
  if (current.app && current.active === null && !current.available && webplayer.isAvailable()) {
    return Object.assign({}, current, { available: true });
  }
  return current;
}
...
async function control(wanted) {
  if (!isAvailable()) throw new Error("aucune application pilotable au premier plan");
  ...
  if (current.active === null) {
    ...
    /* La page d'abord : webkitEnterFullScreen met la vidéo elle-même en
       plein écran, sans dépendre du focus dans la page ni de
       l'Accessibilité. La frappe de touche, elle, n'atteint le lecteur que
       si celui-ci a déjà le focus. */
    if (await webplayer.toggleFullscreen(current.app)) return current;

    /* Sans la page, il reste la frappe, qui exige l'Accessibilité. */
    if (!trusted) {
      throw new Error("ni la page ni l'Accessibilité ne permettent le plein écran");
    }

    const issue = await applescript(KEYSTROKE);
    ...
    /* La garde n'a pas pu se prononcer. On s'abstient plutôt que de frapper
       à l'aveugle : une garde qui échoue en silence ne protège de rien, et
       c'est exactement le défaut qu'elle est censée éviter. Le numéro
       d'erreur AppleScript est conservé, seul moyen de distinguer un refus
       d'autorisation d'une fenêtre sans élément focalisé. */
    if (issue !== "0") {
      throw new Error("lecture du focus impossible (" + issue + ")");
    }
