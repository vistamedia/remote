...
/* Empreinte de l'interface, posée par le serveur : le client compare
   celle qu'il porte à celle-ci et se recharge quand elles diffèrent. Sans
   cela, le cache d'une semaine — nécessaire pour que la télécommande
   s'ouvre sans réseau — figerait l'interface sur l'iPhone. */
let interfaceVersion = null;

function setVersion(valeur) {
  interfaceVersion = valeur;
}
...
function compose(base) {
  ...
  state.version = interfaceVersion;
...
/* Le badge nomme déjà la plateforme : la répéter en tête du titre n'apprend
   rien. Prime Video préfixe systématiquement le sien — « Prime Video: Ride or
   Die ». */
function sansPrefixe(titre, source) {
  if (!titre || !source) return titre;
  const prefixe = (source + ":").toLowerCase();
  if (titre.toLowerCase().indexOf(prefixe) !== 0) return titre;
  /* On ne rend le titre nu que s'il reste quelque chose à nommer. */
  return titre.slice(prefixe.length).trim() || titre;
}
...
function withPage(snapshot) {
  ...
  return Object.assign({}, snapshot, {
    ...
    /* La page l'emporte dès qu'elle nomme quelque chose : elle sépare
       l'œuvre de l'épisode, là où le système livre un seul libellé. Le
       préfixe de plateforme est retiré quelle que soit la provenance — le
       titre de l'onglet de Prime Video le porte aussi. */
    title: sansPrefixe(page.title || snapshot.title, source),
...
function differs(a, b) {
  ...
  if (a.volume !== b.volume
    ...
    || a.brightnessControllable !== b.brightnessControllable
    || a.version !== b.version) return true;
...
module.exports = { setVersion, compose, publish, start, stop, isPolling };
